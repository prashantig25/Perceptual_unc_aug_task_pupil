function h = bar_plots_pval(y, mean_all, SEM_all, n,x_groups,bars,...
    legend_names, xticks, xticklabs, title_name, xlabelname, ylabelname,...
    disp_pval, scatter_dots, dot_size, plot_err, fontsize, linewidth,...
    fontname, disp_legend, varargin)
    ...
    %   varargin{7}: Custom legend location
    ...
    if disp_legend == 1
        l = legend(legend_names,"AutoUpdate","off",'Location','best','Box','off','Color', ...
            'none','EdgeColor','none', 'FontSize', fontsize);
    ...
    % input argument
    if disp_legend == 1 && length(varargin) >= 7
        l.Position = varargin{7};
    end
end