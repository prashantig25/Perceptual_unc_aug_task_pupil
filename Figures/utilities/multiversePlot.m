function multiversePlot(coeffStringHet, coeffStringNormal)
%MULTIVERSEPLOT Plots the multiverse-style analysis for the coefficient 
% of interest
%
%   INPUT
%    coeffStringHet: Coefficient name (string) for heteroskedastic analyses
%    coeffStringNormal: Coefficient name (string) for regular analyses
...
coeff_idx_het = find(strcmp(het_coeff_names, coeffStringHet));
...
coeff_idx_norm  = find(strcmp(ols_coeff_names, coeffStringNormal));
...
pe_condiff_linearInt(sub_id,col_id) = betas_linearInt.with_intercept(1,coeff_idx_norm,sub_id,col_id);
pe_condiff_cubicSpline(sub_id,col_id) = betas_cubicSpline.with_intercept(1,coeff_idx_norm,sub_id,col_id);
pe_condiff_deconv(sub_id,col_id) = betas_deconv.with_intercept(1,coeff_idx_norm,sub_id,col_id);
pe_condiff_linearInt_RT(sub_id,col_id) = betas_linearInt_RT.with_intercept(1,coeff_idx_norm,sub_id,col_id);
pe_condiff_cubicSpline_RT(sub_id,col_id) = betas_cubicSpline_RT.with_intercept(1,coeff_idx_norm,sub_id,col_id);
pe_condiff_deconv_RT(sub_id,col_id) = betas_deconv_RT.with_intercept(1,coeff_idx_norm,sub_id,col_id);
pe_condiff_linearInt_noBL(sub_id,col_id) = betas_linearInt_noBL.with_intercept(1,coeff_idx_norm,sub_id,col_id);
pe_condiff_cubicSpline_noBL(sub_id,col_id) = betas_cubicSpline_noBL.with_intercept(1,coeff_idx_norm,sub_id,col_id);
pe_condiff_deconv_noBL(sub_id,col_id) = betas_deconv_noBL.with_intercept(1,coeff_idx_norm,sub_id,col_id);

% Heteroskedasticity specifications (coefficient 6)
pe_condiff_linear_het(sub_id,col_id) = betas_linear_het.with_intercept(1,coeff_idx_het,sub_id,col_id);
pe_condiff_cubic_het(sub_id,col_id) = betas_cubic_het.with_intercept(1,coeff_idx_het,sub_id,col_id);
pe_condiff_deconv_het(sub_id,col_id) = betas_deconv_het.with_intercept(1,coeff_idx_het,sub_id,col_id);
...
% Original 9
pecondiff_pval_linearInt = perm_linearInt.mask(coeff_idx_norm,:);
pecondiff_pval_cubicSpline = perm_cubicSpline.mask(coeff_idx_norm,:);
pecondiff_pval_deconv = perm_deconv.mask(coeff_idx_norm,:);
pecondiff_pval_linearInt_RT = perm_linearInt_RT.mask(coeff_idx_norm,:);
pecondiff_pval_cubicSpline_RT = perm_cubicSpline_RT.mask(coeff_idx_norm,:);
pecondiff_pval_deconv_RT = perm_deconv_RT.mask(coeff_idx_norm,:);
pecondiff_pval_linearInt_noBL = perm_linearInt_noBL.mask(coeff_idx_norm,:);
pecondiff_pval_cubicSpline_noBL = perm_cubicSpline_noBL.mask(coeff_idx_norm,:);
pecondiff_pval_deconv_noBL = perm_deconv_noBL.mask(coeff_idx_norm,:);

% Heteroskedasticity
pecondiff_pval_linear_het = perm_linear_het.mask(coeff_idx_het,:);
pecondiff_pval_cubic_het = perm_cubic_het.mask(coeff_idx_het,:);
pecondiff_pval_deconv_het = perm_deconv_het.mask(coeff_idx_het,:);

% P-values - original 9
pecondiff_prob_linearInt = min(unique(perm_linearInt.prob(coeff_idx_norm,:)));
pecondiff_prob_cubicSpline = min(unique(perm_cubicSpline.prob(coeff_idx_norm,:)));
pecondiff_prob_deconv = min(unique(perm_deconv.prob(coeff_idx_norm,:)));
pecondiff_prob_linearInt_RT = min(unique(perm_linearInt_RT.prob(coeff_idx_norm,:)));
pecondiff_prob_cubicSpline_RT = min(unique(perm_cubicSpline_RT.prob(coeff_idx_norm,:)));
pecondiff_prob_deconv_RT = min(unique(perm_deconv_RT.prob(coeff_idx_norm,:)));
pecondiff_prob_linearInt_noBL = min(unique(perm_linearInt_noBL.prob(coeff_idx_norm,:)));
pecondiff_prob_cubicSpline_noBL = min(unique(perm_cubicSpline_noBL.prob(coeff_idx_norm,:)));
pecondiff_prob_deconv_noBL = min(unique(perm_deconv_noBL.prob(coeff_idx_norm,:)));

% P-values - heteroskedasticity
pecondiff_prob_linear_het = min(unique(perm_linear_het.prob(coeff_idx_het,:)));
pecondiff_prob_cubic_het = min(unique(perm_cubic_het.prob(coeff_idx_het,:)));
pecondiff_prob_deconv_het = min(unique(perm_deconv_het.prob(coeff_idx_het,:)));
...
title('Multiverse-style analysis: All 12 specifications','FontName',fontname,'FontSize',fontsize+1,'FontWeight','normal')
...
text(0.25, 0.028, 'Significance threshold for two-tailed test', 'FontSize', 7, 'VerticalAlignment', 'middle');
text(0.25, 0.053, 'Significance threshold for one-tailed test', 'FontSize', 7, 'VerticalAlignment', 'middle');
