...
for n = 1:num_vars
    ...
    if betas == 1 % if perm test is to be conducted on regression betas
        ...
        grandavg_cat1.beta = reshape(squeeze(var1(1, var(n), :, :)), [num_subjs, 1, col]);
        if two_tailed == 1
            grandavg_cat2.beta = reshape(squeeze(var2(2, var(n), :, :)), [num_subjs, 1, col]);
        else
            grandavg_cat2.beta = zeros(num_subjs, 1, col);
        end
        ...
        grandavg_cat1.beta = reshape(var1, [num_subjs, 1, col]);
        grandavg_cat2.beta = reshape(var2, [num_subjs, 1, col]);
    end
