...
hold on 
plot(xaxis,mean(interaction.subj_pupil(1,2).signal) - mean(interaction.subj_pupil(1,1).signal),'Color',high_PU,'LineWidth',1.5)
plot(xaxis,mean(interaction.subj_pupil(2,2).signal) - mean(interaction.subj_pupil(2,1).signal),'Color',low_PU,'LineWidth',1.5)
...
l.String = {['Contrast-difference range = [0 - 0.05]'], ...
            ['Contrast-difference range = [0.05 - 0.1]']};
...
ylabel({'Posterior-predicted difference curves', '(high - low PE)'});
hold on
% a1 = annotation("arrow",[0.78,0.78],[0.27,0.37],'LineWidth',0.5,'Color',low_PU);
% a2 = annotation("arrow",[0.78,0.78],[0.17,0.27],'LineWidth',0.5,'Color',high_PU);
% a1.HeadLength = 5; a2.HeadLength = 5;
