...
betas_struct = importdata(strcat(desiredPath, filesep, "data", filesep, "GB data two pipelines", filesep, "pupil", filesep, "regression", filesep, "main", filesep,"pe_condiff5bins_linearInt.mat")); % add PE bin curves
...
% Replace the old color_cell definition
color_cell = { cols(1,:); cols(2,:); cols(3,:); cols(4,:); cols(5,:) };col = 300; m = 0; start = 0;
...
xlim([-300,2700])
% ylim(ylim_axes)
l = legend('Bin 1 state uncertainty','Bin 2 state uncertainty','Bin 3 state uncertainty','Bin 4 state uncertainty','Bin 5 state uncertainty','Location','best','EdgeColor', ...
