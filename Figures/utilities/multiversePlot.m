function multiversePlot(coeff, width_cm, height_cm)
%MULTIVERSEPLOT Plots the multiverse-style analysis for the coefficient
% of interest
%
%   INPUT
%    coeff: Coefficient name
%    width_cm: Plot width in cm
%    height_cm: Plot height in cm

% INITIALIZE VARS
if strcmp(coeff, 'pe')
    norm_coeff = 'pe';
    het_coeff = 'PE';
    coeff_ylabel = 'PE-modulated pupil';
elseif strcmp(coeff, 'pe_condiff')
    norm_coeff = 'zsc_condiff:pe';
    het_coeff = 'PExCondiff';
    coeff_ylabel = 'Uncertainty-weighted PE';
end
...
coeff_het = find(strcmp(het_coeff_names, het_coeff));
...
betas_deconv           = importdata(fullfile(alt_dir,  'pe_condiff_deconvolution.mat'));
perm_deconv            = importdata(fullfile(alt_dir,  'perm_pe_condiff_deconvolution.mat'));
...
betas_linearInt_noBL   = importdata(fullfile(alt_dir,  'pe_condiff_nonBaselineCorrected_linearInt.mat'));
perm_linearInt_noBL    = importdata(fullfile(alt_dir,  'perm_pe_condiff_nonBaselineCorrected_linearInt.mat'));
betas_cubicSpline_noBL = importdata(fullfile(alt_dir,  'pe_condiff_nonBaselineCorrected_cubicSplineNew.mat'));
perm_cubicSpline_noBL  = importdata(fullfile(alt_dir,  'perm_pe_condiff_nonBaselineCorrected_cubicSplineNew.mat'));
...
coeff_norm  = find(strcmp(ols_coeff_names, norm_coeff));
...
pe_condiff_linearInt(sub_id,col_id) = betas_linearInt.with_intercept(1,coeff_norm,sub_id,col_id);
pe_condiff_cubicSpline(sub_id,col_id) = betas_cubicSpline.with_intercept(1,coeff_norm,sub_id,col_id);
pe_condiff_deconv(sub_id,col_id) = betas_deconv.with_intercept(1,coeff_norm,sub_id,col_id);
pe_condiff_linearInt_RT(sub_id,col_id) = betas_linearInt_RT.with_intercept(1,coeff_norm,sub_id,col_id);
pe_condiff_cubicSpline_RT(sub_id,col_id) = betas_cubicSpline_RT.with_intercept(1,coeff_norm,sub_id,col_id);
pe_condiff_deconv_RT(sub_id,col_id) = betas_deconv_RT.with_intercept(1,coeff_norm,sub_id,col_id);
pe_condiff_linearInt_noBL(sub_id,col_id) = betas_linearInt_noBL.with_intercept(1,coeff_norm,sub_id,col_id);
pe_condiff_cubicSpline_noBL(sub_id,col_id) = betas_cubicSpline_noBL.with_intercept(1,coeff_norm,sub_id,col_id);
pe_condiff_deconv_noBL(sub_id,col_id) = betas_deconv_noBL.with_intercept(1,coeff_norm,sub_id,col_id);

% Heteroskedasticity specifications (coefficient 6)
pe_condiff_linear_het(sub_id,col_id) = betas_linear_het.with_intercept(1,coeff_het,sub_id,col_id);
pe_condiff_cubic_het(sub_id,col_id) = betas_cubic_het.with_intercept(1,coeff_het,sub_id,col_id);
pe_condiff_deconv_het(sub_id,col_id) = betas_deconv_het.with_intercept(1,coeff_het,sub_id,col_id);
...
% Original 9
pecondiff_pval_linearInt = perm_linearInt.mask(coeff_norm,:);
pecondiff_pval_cubicSpline = perm_cubicSpline.mask(coeff_norm,:);
pecondiff_pval_deconv = perm_deconv.mask(coeff_norm,:);
pecondiff_pval_linearInt_RT = perm_linearInt_RT.mask(coeff_norm,:);
pecondiff_pval_cubicSpline_RT = perm_cubicSpline_RT.mask(coeff_norm,:);
pecondiff_pval_deconv_RT = perm_deconv_RT.mask(coeff_norm,:);
pecondiff_pval_linearInt_noBL = perm_linearInt_noBL.mask(coeff_norm,:);
pecondiff_pval_cubicSpline_noBL = perm_cubicSpline_noBL.mask(coeff_norm,:);
pecondiff_pval_deconv_noBL = perm_deconv_noBL.mask(coeff_norm,:);

% Heteroskedasticity
pecondiff_pval_linear_het = perm_linear_het.mask(coeff_het,:);
pecondiff_pval_cubic_het = perm_cubic_het.mask(coeff_het,:);
pecondiff_pval_deconv_het = perm_deconv_het.mask(coeff_het,:);

% P-values - original 9
pecondiff_prob_linearInt = min(unique(perm_linearInt.prob(coeff_norm,:)));
pecondiff_prob_cubicSpline = min(unique(perm_cubicSpline.prob(coeff_norm,:)));
pecondiff_prob_deconv = min(unique(perm_deconv.prob(coeff_norm,:)));
pecondiff_prob_linearInt_RT = min(unique(perm_linearInt_RT.prob(coeff_norm,:)));
pecondiff_prob_cubicSpline_RT = min(unique(perm_cubicSpline_RT.prob(coeff_norm,:)));
pecondiff_prob_deconv_RT = min(unique(perm_deconv_RT.prob(coeff_norm,:)));
pecondiff_prob_linearInt_noBL = min(unique(perm_linearInt_noBL.prob(coeff_norm,:)));
pecondiff_prob_cubicSpline_noBL = min(unique(perm_cubicSpline_noBL.prob(coeff_norm,:)));
pecondiff_prob_deconv_noBL = min(unique(perm_deconv_noBL.prob(coeff_norm,:)));

% P-values - heteroskedasticity
pecondiff_prob_linear_het = min(unique(perm_linear_het.prob(coeff_het,:)));
pecondiff_prob_cubic_het = min(unique(perm_cubic_het.prob(coeff_het,:)));
pecondiff_prob_deconv_het = min(unique(perm_deconv_het.prob(coeff_het,:)));
...
plot(xaxis, mean7, 'Color', color_linear_noBL, 'LineWidth', 2, 'LineStyle', '-', 'DisplayName', 'Linear no baseline');
plot(xaxis, mean8, 'Color', color_cubic_noBL, 'LineWidth', 2, 'LineStyle', '-', 'DisplayName', 'Cubic no baseline');
plot(xaxis, mean9, 'Color', color_deconv_noBL, 'LineWidth', 2, 'LineStyle', '-', 'DisplayName', 'Deconvolution no baseline');
...
plot(xaxis, mean12, 'Color', color_deconv_hetero, 'LineWidth', 2, 'LineStyle', '-', 'DisplayName', 'Deconvolution heteroskedastic');
...
text(0.5, 0.056, {'Significance threshold', 'one-tailed test'}, 'FontSize', 7, 'VerticalAlignment', 'middle');
text(0.5, 0.033, {'Significance threshold', 'two-tailed test'}, 'FontSize', 7, 'VerticalAlignment', 'middle');
