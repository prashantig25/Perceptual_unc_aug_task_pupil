...
num_subs = 2; %length(subj_ids); % number of subjects
...
base_trialspecific = 1; % get baseline signal for that trial
...
save_dir = strcat(desiredPath, filesep,'data', filesep,'GB data',filesep, 'pupil', filesep, 'pupil signal', filesep, 'fb full trial'); 
...
for s = 1:num_subs
    % LOOP OVER SESSIONS
    for ss = 1:num_sess(s)
        pupil = run_PupilSignal(num_sess,subj_ids,behv_dir,used_preprocessed, ...
...
% FB-LOCKED PUPIL SIGNAL CORRECTED WITH  SPECIFIC BASELINE

base_trialspecific = 0; % event specific baseline
save_dir = strcat(desiredPath, filesep,'data', filesep,'GB data',filesep, 'pupil', filesep, 'pupil signal', filesep, 'fb'); 
...
save_dir = strcat(desiredPath, filesep,'data', filesep,'GB data',filesep, 'pupil', filesep, 'pupil signal', filesep, 'patch'); 
...
save_dir = strcat(desiredPath, filesep,'data', filesep,'GB data',filesep, 'pupil', filesep, 'pupil signal', filesep, 'resp'); 
